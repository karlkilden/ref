...
import static com.kildeen.ref.application.RollBackException.Cause.OptimisticLock;
...
public class FactMapper extends SimpleQueryInOutMapperBase<Fact, FactDTO> {
    ...
    protected Fact toEntity(final FactDTO dto) {
        ...
        if (dto.getId() != 0) {
            ...
            if (dto.getVersion()< fact.getVersion()) {
                throw new RollBackException(fact, OptimisticLock);
            }
        } else {
            fact = new Fact();
        }
